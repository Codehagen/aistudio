...
Proppi is an AI-powered real estate photo editing SaaS platform built with Next.js 16. Users upload property photos, select style templates, and the AI transforms them into marketing-ready visuals. The platform also supports AI video generation from property images.

## Development Commands

```bash
# Start development (requires BOTH terminals)
pnpm dev              # Terminal 1: Next.js dev server
pnpm trigger          # Terminal 2: Trigger.dev background jobs
...
- **Auth**: Better Auth with email/password, workspace roles (owner/admin/member)
- **AI**: Fal.ai (Nano Banana Pro for editing, Qwen for inpainting)
- **Background Jobs**: Trigger.dev v4
- **Storage**: Supabase Storage for images/videos
- **Payments**: Stripe (international) + Fiken invoicing (Norwegian B2B)
...
├── video/              # Video creation features
├── admin/              # Super admin panel (requires isSystemAdmin)
├── api/                # API routes (auth, webhooks, processing endpoints)
lib/
├── db/schema.ts        # Drizzle schema (all tables defined here)
├── db/queries.ts       # Database query functions
├── auth.ts             # Better Auth configuration
├── style-templates.ts  # AI style prompts for image editing
├── room-types.ts       # 27 room type definitions
├── fal.ts              # Fal.ai client setup
├── supabase.ts         # Storage operations
trigger/                # Trigger.dev task definitions
├── process-image.ts    # Core image AI processing
├── inpaint-image.ts    # Mask-based editing
├── video-*.ts          # Video generation pipeline
```

### Data Model
- **Workspace**: Multi-tenant container (has users, projects, billing)
- **User**: Belongs to workspace with role, can be system admin
- **Project**: Groups multiple image generations with shared style/room type
- **ImageGeneration**: Individual AI-processed image with version history
- **VideoProject/VideoClip**: Video generation with per-clip status tracking

### Background Jobs (Trigger.dev v4)
Tasks in `/trigger/` handle long-running AI operations:
- `process-image`: Fetches image → sends to Fal.ai → saves result
- `inpaint-image`: Mask-based regeneration using Qwen
- `generate-video-clip`, `compile-video`, `video-orchestrator`: Video pipeline

Key patterns:
- Always check `result.ok` before accessing `result.output` from `triggerAndWait()`
- Never use `Promise.all` with `triggerAndWait` or `batchTriggerAndWait`
- Use `import type` for task type references

### Auth & Permissions
- Workspace-scoped: users see only their workspace's data
- Roles: `owner` > `admin` > `member`
- System admins (`isSystemAdmin: true`) access `/admin/*` routes
- Admin impersonation tracked via `session.impersonatedBy`

### Norwegian B2B Features
- Organization number validation (9 digits)
- Invoice eligibility (`invoiceEligible` flag)
- Fiken accounting integration for automated invoicing
- Currency: NOK (stored in ore, 100 ore = 1 krone)

## Code Style

- Uses Biome for formatting/linting (config in `biome.jsonc`)
- Post-edit hook runs `pnpm dlx ultracite fix` automatically
